...
func UpdateSDInstanceGroup(c *websocket.Client, msg websocket.WebSocketMessage) {
	...
	if !ok {
		c.SafeSend(websocket.WebSocketMessage{Type: websocket.MessageResponse, ID: msg.ID, Error: "missing input"})
		return
	}

	bytes, err := json.Marshal(inputRaw)
	if err != nil {
		c.SafeSend(websocket.WebSocketMessage{Type: websocket.MessageResponse, ID: msg.ID, Error: "invalid input"})
		return
	}
