...
func kickstartISC() {
	...
	sharedUtils.TerminateOnError(isc.EnsureIngestQueuesForAllSDTypes(rabbitMQClient), "[ISC setup] Failed to ensure ingest queues for existing SD types")
...
func main() {
	...
	err := dbClient.GetRelationalDatabaseClientInstance().PerformOnStartupOperations(auth.RolePermissions, auth.SystemRoleUID, auth.FormatPermissionLabel)
